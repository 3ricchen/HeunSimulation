...
def eigvis(filename, res=10, half=100, axis=False):
    '''
    eigvis
    =====================
    ...
    for line in data:
        ...
        for cord in line:
            try:
                ...
                real_pix = x * width // len(data)
                imag_pix = (len(data) - 1 - y) * width // len(data)
                if abs(real_eig - real_pix) < width//len(data) and abs(imag_eig - imag_pix) < width//len(data):
                    # Uncomment this to draw grey circles at each of the eigenvalu positions
                    pygame.draw.circle(screen, (31, 31, 31), [real_eig, imag_eig], 1.5*width // len(data))
                    ...
                    #                    [(5*float(val.real) * res + half) * width // len(data), (5*-float(val.imag) * res + half) * width // len(data), 2*width // len(data), 2*width // len(data)])
                    pass
            except:
                pass
            y += 1
    ...
    if axis:
        scaling = 1 # Set to 5 if only drawing the square roots of convergence values
        ...
        font18 = pygame.font.Font(pygame.font.get_default_font(),18)
        screen.blit(font18.render(str(half/2/res/scaling)+"i",False,(0,0,255),None),[width//2+4,width//4-9])
        screen.blit(font18.render(str(-half /2/res/scaling), False, (0, 0, 255), None),
                    [width // 4 -20, width // 2 - 18])
        screen.blit(font18.render(str(-half /2/res/scaling)+"i", False, (0, 0, 255), None),
                    [width // 2 + 4, 3*width // 4 -9])
        screen.blit(font18.render(str(half /2/res/scaling), False, (0, 0, 255), None),
                    [3*width // 4 - 15, width // 2 - 18])
    pygame.display.update()

    pygame.image.save(screen, "./" + filename + "hsvaE.png") # Change to edit the output filename

eigvis("Data/eigenvaluedata200_eigvalsH-1", half=200, res=4) # Produces the image for the Lame Equation
while flag:
